...
export class Heatmap {
  ...
  get calendar() {
    if (!this._calendar) {
      ...
      this._calendar = Array.from({ length: this.weekCount })

      for (let i = 0, len = this._calendar.length; i < len; i++) {
        // this._calendar[i] = new Array(Heatmap.DAYS_IN_WEEK)
        this._calendar[i] = Array.from({ length: Heatmap.DAYS_IN_WEEK })
...
// export function heatmap(endDateVal: Date | string, values: Value[], maxVal?: number) {
//   let startDate: Date
//   let endDate: Date = parseDate(endDateVal)
//   let max: number

//   const _values: Value[]
//   const _firstFullWeekOfMonths: Month[]
...
// const ordered = [1, 8, 15, 22, 29, 2,  9, 16, 23, 30, 3, 10, 17, 24, 31, 4, 11, 18, 25, null, 5, 12, 19, 26, null, 6, 13, 20, 27, null, 7, 14, 21, 28, null]
